...
You will need a Unix-like environment to build the firmware (see below for instructions on how to get that on Windows, macOS, and Linux).

* install `arm-none-eabi-gcc` (we've been using `9-2019-q4-major` and recently switched to `10-2020-q4-major`)
* install `node.js`
* install GNU Make
...
There are verification errors with brew packages for arm-gcc, so we recommend
...
Once you install it, you either need to add `/Applications/ARM/bin` to your `PATH`, or symlink binaries like this:
`cd /usr/local/bin; ln -sf /Applications/ARM/bin/arm-none-eabi-* .`

Node.js can be [downloaded from the usual place](https://nodejs.org/en/download/).
